...
- **`KeystrokeSimulator` fallback для терминалов:** в Terminal.app/iTerm2 видимый текст — рендер PTY, а не редактируемая AX-строка; `AXUIElementSetAttributeValue` может отрапортовать успех, ничего не применив. Когда AX-путь не сработал (`applyCorrection` в `InputMonitor`) и фронтальное приложение — известный терминал (`KeystrokeSimulator.isTerminalFrontmost`), вместо AX используются синтетические keystroke-события (Backspace × N + один Unicode-string keystroke). Каждое такое событие помечено `eventSourceUserData = KeystrokeSimulator.eventMarker` — `InputMonitor.handle` проверяет этот тег первым делом и игнорирует такие события, иначе они испортили бы буфер слова или вызвали повторную коррекцию. Из-за `correctionDelay` граничный символ (пробел, пунктуация, Enter и т.д.) к моменту срабатывания уже обработан приложением и стоит на экране, а курсор — сразу за ним, поэтому ретроактивный fallback удаляет `word.count + 1` символ и печатает `converted + boundary`, а не просто `converted` (иначе теряется первая буква слова). Если граница — Enter, fallback пропускается целиком: строка уже отправлена шеллу, и к моменту срабатывания фокус мог уйти на новый промпт или в программу, запущенную этой командой — слепая инъекция туда небезопасна. Полное обоснование дизайна — `docs/superpowers/specs/2026-08-12-terminal-keystroke-fallback-design.md`.
